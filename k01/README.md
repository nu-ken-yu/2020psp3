...
## ソースコードの説明
リニアサーチでは配列を左から順に探索していき、目的のIDを見つけるとそこでリターン。
バイナリサーチでは定数MAX_CITYを２で割ったもので配列の真ん中を表す変数midを初期化する。
83行目の条件分は、<=としなければ配列の橋のデータを探索することができない。
配列のデータを昇順にソートされているため、目的とするデータがmid番目のデータより小さければrightをmid - 1に更新し、大きければleftをmid + 1に更新。
93行目は(left + right) / 2と数学的な意味では同じだが、このようにするとオーバーフローする可能背が高くなってしまう。
## 出力結果
